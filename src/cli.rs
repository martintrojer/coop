...
/// Tokenize only enough shell syntax to avoid warning on quoted text, nested
/// pipelines, and pipelines that feed another command. This is deliberately
/// not a shell parser: uncertain forms get no warning rather than a noisy one.
fn dispatch_warnings(command: &str, has_runtime_cap: bool) -> Vec<DispatchWarning> {
